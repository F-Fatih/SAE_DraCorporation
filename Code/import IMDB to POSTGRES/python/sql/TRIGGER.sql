...
-- TRIGGER known title table namebasics
CREATE or replace FUNCTION knownTitleCheck() --Check si les tconst dans l'array sont présents dans la table titlebasics
    RETURNS TRIGGER AS
$$
    DECLARE
        t titlebasics.tconst%TYPE;
		tconstachecker titlebasics.tconst%TYPE;
    BEGIN
		IF NEW.knownfortitles is NULL THEN
			return NEW;
		END IF;
		
		FOR tconstachecker IN SELECT unnest(NEW.knownfortitles)
		LOOP
			SELECT tconst INTO t
				FROM titlebasics
				WHERE tconst = tconstachecker;
			IF NOT FOUND THEN
				RETURN NULL;
        	END IF;
		END LOOP;
...
DROP TRIGGER IF EXISTS namebasics_check_knownfortitle on namebasics;
CREATE TRIGGER namebasics_check_knownfortitle
    BEFORE INSERT ON namebasics
    FOR EACH row
    EXECUTE PROCEDURE knownTitleCheck();



--array nconst de crew
CREATE or replace FUNCTION arrayNconstCheck() --Check si les nconst dans l'array sont présents dans la table namebasics
    RETURNS TRIGGER AS
$$
    DECLARE
        n namebasics.nconst%TYPE;
		nconstachecker namebasics.nconst%TYPE;

    BEGIN
		IF NEW.directors is NULL and NEW.writers is NULL THEN
			return NEW;
		END IF;
		
  IF NEW.directors is not null THEN
		 FOR nconstachecker IN SELECT unnest(NEW.directors)
		 LOOP
			 SELECT nconst INTO n
				 FROM namebasics
				 WHERE nconst = nconstachecker;
			 IF NOT FOUND THEN
				 RETURN NULL;
    	END IF;
		  END LOOP;
  ...
  IF NEW.writers is not null THEN
		 FOR nconstachecker IN SELECT unnest(NEW.writers)
		 LOOP
			 SELECT nconst INTO n
				 FROM namebasics
				 WHERE nconst = nconstachecker;
...
DROP TRIGGER IF EXISTS crew_ckeck_array on titlecrew;
CREATE TRIGGER crew_ckeck_array
    BEFORE INSERT ON titlecrew
    FOR EACH row
    EXECUTE PROCEDURE arrayNconstCheck();
